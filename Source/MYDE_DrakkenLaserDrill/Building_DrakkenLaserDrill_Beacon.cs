...
namespace MYDE_DrakkenLaserDrill
{
    ...
    public class Building_DrakkenLaserDrill_Beacon : ThingWithComps
    {
        ...
        public void CheckSpawn(Building_DrakkenLaserDrill buildingDrakkenLaserDrill, Vector2 realPos,
            List<Thing> listThing, Thing targetThing)
        {
        ...
        protected override void Tick()
        {
            ...
            if (TakeDamageTick >= TakeDamageTickMax && LaserScaleTick >= 30)
            {
                ...
                if (TakeDamageBool)
                {
                    ...
                    Building_DrakkenLaserDrill.DamageTarget();
                }
            }
                ...
                for (var i = 0; i < ListThing.Count; i++)
                {
                    if (ListThing[i] is Pawn)
                    {
                        ...
                        if (pawn is not { Dead: true })
                        {
                            if (pawn?.Map == Map)
                            {
                                if (pawn is not { Downed: true } || Building_DrakkenLaserDrill.IfAttackDown)
                                {
                                    list.Add(pawn);
                                }
        ...
        // Get_MinLaserPos_A_Pos ~ F_Pos 는 그대로 유지됩니다.
